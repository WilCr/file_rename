...
export async function sendPasswordResetEmail(to, resetUrl) {
  ...
  if (!isValidFromAddress(from)) {
    return {
      ...
      reason: 'invalid_from',
      message:
        'EMAIL_FROM must be an email address (e.g. ScanRename <onboarding@resend.dev>), not a website URL.',
    }
  }
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      from,
      to,
      subject: 'Reset your ScanRename password',
      html: `<p>Click the link below to choose a new ScanRename password. It expires in one hour.</p>
