...
export default function PricingModal({ isOpen, onClose }) {
  ...
  const paidPlansReady = Boolean(PRICE_PRO && PRICE_BUSINESS)
      ...
      <div className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-xl sm:p-8">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-3">
          {plans.map((plan) => {
            ...
            const canUpgrade = Boolean(plan.priceId)
            let buttonLabel = 'Coming soon'
            if (isFree) buttonLabel = 'Current plan'
            else if (canUpgrade) buttonLabel = 'Upgrade now'
